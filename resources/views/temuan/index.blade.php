...
<div class="py-6 max-w-7xl mx-auto space-y-4">
    ...
    <div class="flex items-center justify-between">
        <p class="text-sm text-gray-400 dark:text-gray-500">Kelola daftar lokasi dan status temuan</p>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4">
        <form method="GET" class="flex flex-wrap gap-3 items-end">
            <div class="flex-1 min-w-48">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Nomor, lokasi, kategori..."
            ...
            <div class="flex gap-2">
                <button class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors">Cari</button>
                @if(request('search'))
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-1.5">
                                ...
                                {{-- Detail --}}
                                <a href="{{ route('temuan.show', $item) }}" title="Detail"
                                ...
                                {{-- Edit --}}
                                <a href="{{ route('temuan.edit', $item) }}" title="Edit"
                                ...
                                {{-- Hapus --}}
                                <form method="POST" action="{{ route('temuan.destroy', $item) }}" class="inline" x-data=""
                                    x-on:submit.prevent="if(confirm('Hapus temuan ini?')) $el.submit()">
                                    @csrf @method('DELETE')
                                    <button type="submit" title="Hapus"
                                        ...
                                        @if($item->ncrs_count > 0) disabled @endif>
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
