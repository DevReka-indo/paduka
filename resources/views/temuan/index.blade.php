...
<div class="py-6 max-w-7xl mx-auto space-y-4">
    ...
    <div class="flex items-center justify-between">
        <p class="text-sm text-gray-400">Kelola daftar lokasi dan status temuan</p>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
        <form method="GET" action="{{ route('temuan.index') }}" class="flex flex-wrap gap-3 items-end">
            <div class="flex-1 min-w-48">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Nomor, lokasi, atau kategori temuan..."
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100 text-xs uppercase tracking-wide text-gray-400">
                        ...
                        <th class="px-4 py-3 text-left font-semibold">Nomor Temuan</th>
                        <th class="px-4 py-3 text-left font-semibold">Lokasi Temuan</th>
                        <th class="px-4 py-3 text-left font-semibold">Kategori Temuan</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-gray-400">{{ $temuans->firstItem() + $loop->index }}</td>

                        <td class="px-4 py-3">
                            <a href="{{ route('temuan.show', $item) }}"
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center text-xs font-semibold bg-blue-50 text-blue-600 px-2.5 py-1 rounded-full">
                                {{ $item->status_temuan }}
                            </span>
                        </td>

                        <td class="px-4 py-3 text-gray-600 max-w-xs">
                            <p class="truncate">{{ $item->detail_temuan ?: '-' }}</p>
                        </td>

                        <td class="px-4 py-3">
                            <span class="inline-flex items-center gap-1 text-xs font-semibold
                                {{ $item->ncrs_count > 0 ? 'bg-orange-50 text-orange-600' : 'bg-gray-100 text-gray-400' }}
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-1.5">
                                ...
                                <form action="{{ route('temuan.destroy', $item) }}" method="POST" class="inline"
                                    x-data=""
                                    x-on:submit.prevent="if(confirm('Hapus temuan {{ $item->nomor_temuan }}?\nPastikan tidak ada NCR yang terkait.')) $el.submit()">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        ...
                                        {{ $item->ncrs_count > 0 ? 'opacity-40 cursor-not-allowed' : '' }}"
                                        @if($item->ncrs_count > 0) disabled title="Tidak dapat dihapus, masih ada NCR terkait" @endif>
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr>
                        <td colspan="6" class="px-4 py-12 text-center">
                            <div class="flex flex-col items-center gap-2 text-gray-300">
                                ...
                                <p class="text-sm font-medium text-gray-400">Belum ada data lokasi temuan</p>
                                <a href="{{ route('temuan.create') }}" class="text-xs text-indigo-500 hover:underline">
