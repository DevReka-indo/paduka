...
<div class="py-6 max-w-2xl mx-auto space-y-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
            ...
            <div>
                <h3 class="text-sm font-semibold text-gray-800">Form Tambah Lokasi Temuan</h3>
                <p class="text-xs text-gray-400">Isi data lokasi temuan baru</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('temuan.store') }}" class="p-6 space-y-5">
            ...
            <div>
                ...
                <x-text-input id="nomor_temuan_preview" type="text"
                    ...
                    :value="$nomorPreview"
                    readonly />
                <p class="mt-1 text-xs text-gray-400">Nomor temuan akan dibuat otomatis saat data disimpan.</p>
            </div>

            <div>
                <x-input-label for="status_temuan" value="Lokasi Temuan" />
                <x-text-input id="status_temuan" name="status_temuan" type="text"
                    class="mt-1 block w-full" :value="old('status_temuan')"
                    placeholder="Contoh: Lokasi A" required maxlength="25" />
                <x-input-error :messages="$errors->get('status_temuan')" class="mt-2" />
                <p class="mt-1 text-xs text-gray-400">Maksimal 25 karakter.</p>
            </div>

            <div>
                <x-input-label for="detail_temuan" value="Kategori Temuan" />
                <x-text-input id="detail_temuan" name="detail_temuan" type="text"
                    class="mt-1 block w-full" :value="old('detail_temuan')"
                    placeholder="Deskripsikan detail kategori temuan..." maxlength="175" />
            ...
            <div class="flex items-center gap-3 pt-2 border-t border-gray-100">
                <x-primary-button>Simpan Temuan</x-primary-button>
                <a href="{{ route('temuan.index') }}"
