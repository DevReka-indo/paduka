...
@section('content')
    <div class="py-6 px-4 sm:px-6 lg:px-8 w-full max-w-[1600px] mx-auto space-y-4">
        ...
        <div class="mb-6">
            ...
            <h1 class="text-xl font-bold text-slate-800 dark:text-gray-100">Tambah Changelog</h1>
            <p class="text-sm text-slate-500 dark:text-gray-400 mt-1">Tambahkan versi dan poin perubahan baru.</p>
        </div>

        <form method="POST" action="{{ route('changelog.store') }}"
            x-data="{ items: {{ json_encode(old('items', [''])) }} }">
            @csrf
