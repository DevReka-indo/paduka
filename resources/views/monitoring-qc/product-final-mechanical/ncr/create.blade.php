    ...
    <div class="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">
                    Tambah Detail NCR Mekanik
                </h1>

                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    QC Product & Final Inspection - Mekanik
                </p>
            </div>
        ...
        >
            <form
                method="POST"
                action="{{ route(
                    'monitoring-qc.product-final-mechanical.ncr.store'
                ) }}"
                class="space-y-6"
            >
                @csrf
                ...
                @include(
                    'monitoring-qc.product-final-mechanical.ncr.partials.form'
                )

                <div class="flex justify-end gap-3 border-t
                    border-slate-200 pt-6 dark:border-slate-800"
                >
                    <a
                        href="{{ route(
                            'monitoring-qc.product-final-mechanical.ncr.index'
